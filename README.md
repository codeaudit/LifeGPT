...
Conway's Game of Life (Life), a well known algorithm within the broader class of cellular automata (CA), exhibits complex emergent dynamics, with extreme sensitivity to initial conditions. Modeling and predicting such intricate behavior without explicit knowledge of the system's underlying topology presents a significant challenge, motivating the development of algorithms that can generalize across various grid configurations and boundary conditions. In this work, we introduce LifeGPT, a decoder-only generative pretrained transformer (GPT) model with rotary positional embedding (RoPE) and forgetful causal masking (FCM), capable of simulating Life on a toroidal grid without prior knowledge of grid size or periodic boundary conditions. LifeGPT is topology-agnostic with respect to its training data, achieving near-perfect accuracy in capturing the deterministic rules of a 2-dimensional CA system, given sufficiently diverse training data. We show that a single LifeGPT model can be trained simultaneously on grids of varying sizes. Furthermore, we introduce the concept of an autoregressive loop, enabling recursive simulation of Life using LifeGPT. Finally, we propose potential future research directions, such as exploring the use of LifeGPT-like models to address inverse problems in dynamical systems by inferring CA rulesets from real-world data, which could potentially contribute to the development of novel predictive models.
